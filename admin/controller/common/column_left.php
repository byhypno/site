...
class ControllerCommonColumnLeft extends Controller {
	public function index() {
		if (isset($this->request->get['token']) && isset($this->session->data['token']) && ($this->request->get['token'] == $this->session->data['token'])) {
			...
			// EGESER - Toplu ürün aktarım aracı
			if ($this->user->hasPermission('access', 'catalog/product')) {
				$catalog[] = array(
					'name'     => 'Toplu Ürün Aktar',
					'href'     => $this->url->link('tool/egeser_product_importer', 'token=' . $this->session->data['token'], true),
					'children' => array()
				);
			}

			// EGESER - 301 yönlendirme merkezi
			if ($this->user->hasPermission('access', 'catalog/product')) {
				$catalog[] = array(
					'name'     => 'SEO / 301 Yönlendirmeler',
					'href'     => $this->url->link('tool/egeser_redirect_manager', 'token=' . $this->session->data['token'], true),
					'children' => array()
				);
			}
