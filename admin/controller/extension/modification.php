...
class ControllerExtensionModification extends Controller {
	...
	public function refresh($data = array()) {
		...
		// EGESER OCMOD AUDITOR V2.0 - full audit return hooks
		if (!$this->error && isset($this->request->get['egeser_audit_init_id'])) {
			$audit_id = preg_replace('/[^a-zA-Z0-9_-]/', '', $this->request->get['egeser_audit_init_id']);
			$this->response->redirect($this->url->link('tool/egeser_ocmod_test/auditAfterInitial', 'token=' . $this->session->data['token'] . '&audit_id=' . $audit_id, true));
			return;
		}

		if (!$this->error && isset($this->request->get['egeser_audit_toggle_id'])) {
			$audit_id = preg_replace('/[^a-zA-Z0-9_-]/', '', $this->request->get['egeser_audit_toggle_id']);
			$this->response->redirect($this->url->link('tool/egeser_ocmod_test/auditAfterToggle', 'token=' . $this->session->data['token'] . '&audit_id=' . $audit_id, true));
			return;
		}

		if (!$this->error && isset($this->request->get['egeser_audit_restore_id'])) {
			$audit_id = preg_replace('/[^a-zA-Z0-9_-]/', '', $this->request->get['egeser_audit_restore_id']);
			$this->response->redirect($this->url->link('tool/egeser_ocmod_test/auditAfterRestore', 'token=' . $this->session->data['token'] . '&audit_id=' . $audit_id, true));
			return;
		}

		if (!$this->error && isset($this->request->get['egeser_audit_disabled_id'])) {
			$audit_id = preg_replace('/[^a-zA-Z0-9_-]/', '', $this->request->get['egeser_audit_disabled_id']);
			$this->response->redirect($this->url->link('tool/egeser_ocmod_test/auditAfterDisabled', 'token=' . $this->session->data['token'] . '&audit_id=' . $audit_id, true));
			return;
		}

		if (!$this->error && isset($this->request->get['egeser_audit_rollback_id'])) {
			$audit_id = preg_replace('/[^a-zA-Z0-9_-]/', '', $this->request->get['egeser_audit_rollback_id']);
			$this->response->redirect($this->url->link('tool/egeser_ocmod_test/auditAfterRollback', 'token=' . $this->session->data['token'] . '&audit_id=' . $audit_id, true));
			return;
		}

		// EGESER OCMOD FINAL TEMIZLIK V1.0 - refresh return hooks
		if (!$this->error && isset($this->request->get['egeser_cleanup_id'])) {
			$cleanup_id = preg_replace('/[^a-zA-Z0-9_-]/', '', $this->request->get['egeser_cleanup_id']);
			$this->response->redirect($this->url->link('tool/egeser_ocmod_test/cleanupResult', 'token=' . $this->session->data['token'] . '&cleanup_id=' . $cleanup_id, true));
			return;
		}

		if (!$this->error && isset($this->request->get['egeser_cleanup_rollback_id'])) {
			$cleanup_id = preg_replace('/[^a-zA-Z0-9_-]/', '', $this->request->get['egeser_cleanup_rollback_id']);
			$this->response->redirect($this->url->link('tool/egeser_ocmod_test/cleanupRollbackResult', 'token=' . $this->session->data['token'] . '&cleanup_id=' . $cleanup_id, true));
			return;
		}

		// EGESER OCMOD TEST PANEL V1.0 - controlled A/B test return hooks
		if (!$this->error && isset($this->request->get['egeser_test_id'])) {
			$test_id = preg_replace('/[^a-zA-Z0-9_-]/', '', $this->request->get['egeser_test_id']);
			$this->response->redirect($this->url->link('tool/egeser_ocmod_test/afterToggle', 'token=' . $this->session->data['token'] . '&test_id=' . $test_id, true));
			return;
		}

		if (!$this->error && isset($this->request->get['egeser_restore_id'])) {
			$test_id = preg_replace('/[^a-zA-Z0-9_-]/', '', $this->request->get['egeser_restore_id']);
			$this->response->redirect($this->url->link('tool/egeser_ocmod_test/result', 'token=' . $this->session->data['token'] . '&test_id=' . $test_id, true));
			return;
		}
	...
	protected function getList() {
		...
		$data['egeser_ocmod_test'] = $this->url->link('tool/egeser_ocmod_test', 'token=' . $this->session->data['token'], true);
		$data['egeser_ocmod_audit'] = $this->url->link('tool/egeser_ocmod_test', 'token=' . $this->session->data['token'] . '#tam-denetim', true);
		$data['egeser_ocmod_cleanup'] = $this->url->link('tool/egeser_ocmod_test/cleanup', 'token=' . $this->session->data['token'], true);
		...
		foreach ($results as $result) {
			$data['modifications'][] = array(
				...
				'test'            => $this->url->link('tool/egeser_ocmod_test', 'token=' . $this->session->data['token'] . '&modification_id=' . $result['modification_id'], true),
