...
class ControllerExtensionModuleEgeserLeadSecurityV102 extends Controller {
    public function nonce() {
        ...
        if (!$this->isAjax() || !(bool)$this->config->get('egeser_lead_security_v102_status')) {
            $this->response->addHeader('HTTP/1.1 403 Forbidden');
            $this->response->setOutput(json_encode(array('success'=>false,'message'=>'Form güvenliği kullanılamıyor.')));
            return;
        }
        ...
        $this->response->setOutput(json_encode(array(
            'success'=>true,
            'csrf_token'=>$this->model_extension_module_egeser_lead_security_v102->issueNonce()
        )));
    }
    ...
    public function submit() {
        ...
        if (!$given || !$this->model_extension_module_egeser_lead_security_v102->verifyNonce($given)) {
            $this->response->addHeader('HTTP/1.1 403 Forbidden');
            $this->response->setOutput(json_encode(array(
                'success'=>false,
                'csrf_refresh'=>true,
        ...
        if (!$this->model_extension_module_egeser_lead_security_v102->consumeNonce($given)) {
            $this->response->addHeader('HTTP/1.1 403 Forbidden');
