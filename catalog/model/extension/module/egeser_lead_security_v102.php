<?php
class ModelExtensionModuleEgeserLeadSecurityV102 extends Model {
    public function ensureTable() {
        $this->db->query("CREATE TABLE IF NOT EXISTS `" . DB_PREFIX . "egeser_csrf_nonce_v102` (
    ...
    public function issueNonce() {
        ...
        $minutes = (int)$this->config->get('egeser_lead_security_v102_lifetime');
        if ($minutes < 5 || $minutes > 60) $minutes = 20;

        $this->db->query("INSERT INTO `" . DB_PREFIX . "egeser_csrf_nonce_v102`
    ...
    public function verifyNonce($token) {
        if (!(bool)$this->config->get('egeser_lead_security_v102_status')) return false;
        ...
        $q = $this->db->query("SELECT `nonce_id` FROM `" . DB_PREFIX . "egeser_csrf_nonce_v102`
    ...
    public function consumeNonce($token) {
        ...
        $this->db->query("UPDATE `" . DB_PREFIX . "egeser_csrf_nonce_v102`
            SET `used`='1'
            WHERE `token_hash`='" . $this->db->escape($hash) . "'
              AND `ua_hash`='" . $this->db->escape($ua) . "'
              AND `used`='0'
    ...
    private function prune() {
        $this->db->query("DELETE FROM `" . DB_PREFIX . "egeser_csrf_nonce_v102`
    ...
    private function randomToken() {
        if (function_exists('random_bytes')) {
            try { return bin2hex(random_bytes(32)); } catch (Exception $e) {}
        }
