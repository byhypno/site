...
class ModelExtensionModuleEgeserLeadSecurityV103 extends Model {
    ...
    public function issueNonce() {
        ...
        $minutes = (int)$this->config->get('egeser_lead_security_v103_lifetime');
        if ($minutes < 5 || $minutes > 60) $minutes = 20;

        $this->db->query("INSERT INTO `" . DB_PREFIX . "egeser_csrf_nonce_v103`
    ...
    public function verifyNonce($token) {
        if (!(bool)$this->config->get('egeser_lead_security_v103_status')) return false;
        ...
        $q = $this->db->query("SELECT `nonce_id` FROM `" . DB_PREFIX . "egeser_csrf_nonce_v103`
    ...
    public function consumeNonce($token) {
        ...
        $this->db->query("UPDATE `" . DB_PREFIX . "egeser_csrf_nonce_v103`
            SET `used`='1'
            WHERE `token_hash`='" . $this->db->escape($hash) . "'
              AND `ua_hash`='" . $this->db->escape($ua) . "'
              AND `used`='0'
    ...
    private function prune() {
        $this->db->query("DELETE FROM `" . DB_PREFIX . "egeser_csrf_nonce_v103`
